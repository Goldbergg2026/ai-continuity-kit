...
**Лёгкий Git-слой для памяти ИИ, контекста, личных знаний и проектов, которые живут дольше одного чата.**
...
Без vector database. Без фонового сервиса. Без обязательного agent framework. Начать можно с обычных Markdown-файлов и Git.
...
3. **Progressive disclosure.** Загружается только контекст, реально нужный для текущей задачи.
...
Second brain обычно сосредоточен на **сборе и поиске знаний**. AI Continuity Kit сосредоточен на **продолжении работы без смешивания памяти, истории, планов и текущей истины**.
...
Никогда не помещайте в Git реальные пароли, токены, приватные ключи, cookies, session state, `.env` и конфигурации с секретами. Вместо значения можно хранить безопасный указатель или имя переменной.
...
Скорее всего проект вам не нужен, если вы используете ИИ только для разовых разговоров или уже имеете зрелую систему, которая решает freshness, ownership, continuity и permissions.
...
**v0.2 preview.** Модель намеренно остаётся лёгкой, пока проверяется на реальных сценариях.
